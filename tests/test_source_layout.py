...
from src.empirical.plotter.figure_utils import find_project_root


ROOT = Path(__file__).resolve().parents[1]
SOURCE = ROOT / "src"
PLOTTER = SOURCE / "empirical" / "plotter"
...
class SourceLayoutTests(unittest.TestCase):
    ...
    def test_notebook_sources_use_current_paths_and_imports(self):
        legacy_path = re.compile(r"src/(?:ablation_imv|multi_imv|shap_imv|gnn|simulated|plotter)/"
                                 r"|src/(?:figure_utils|shared_style)\.py")
        legacy_import = re.compile(r"\b(?:from|import)\s+gnn(?:\.|\s)")
        legacy_plot_import = re.compile(r"^\s*(?:from|import)\s+(?:figure_utils|shared_style)\b",
                                        re.MULTILINE)
        for notebook in notebooks():
            data = json.loads(notebook.read_text())
    ...
    def test_runner_discovers_every_notebook_once_with_plotters_last(self):
        ...
        producers = sorted(path for path in expected if not path.startswith("src/empirical/plotter/"))
        plotters = sorted(path for path in expected if path.startswith("src/empirical/plotter/"))
