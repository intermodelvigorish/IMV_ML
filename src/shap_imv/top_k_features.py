...
* ``SHAP-IMV`` mean exact Shapley value with IMV as the coalition value, read
  from the notebook artefacts written by ``src/shap_imv/*.ipynb`` (recomputed
  with :class:`imv.BinaryIMV` when the artefact is absent).
...
Usage:
    python src/shap_imv/top_k_features.py                 # all three examples
    python src/shap_imv/top_k_features.py --example shap_imv_titanic
"""
...
PROJECT_ROOT = next(
    (path for path in (Path.cwd(), *Path.cwd().parents, Path(__file__).resolve().parents[2])
